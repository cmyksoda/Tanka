...
// Note: Currently wii_audio only supports 16 bit,
...
get_buffers(device_t* device, multi_buffer_list* data)
{
	...
	result = wii_hw_create_virtual_buffers(&device->playback_stream,
				"wii_audio_playback_sem");
	if (result != B_OK) {
		dprintf("wii_audio %s: Error setting up playback buffers (%s)\n",
	...
	result = wii_hw_create_virtual_buffers(&device->record_stream,
				"wii_audio_record_sem");
	if (result != B_OK) {
		dprintf("wii_audio %s: Error setting up recording buffers (%s)\n",
													__func__, strerror(result));
		return result;
	}
...
buffer_exchange(device_t* device, multi_buffer_info* info)
{
	...
	if (!device->running)
		wii_start_hardware(device);

	result = acquire_sem(device->playback_stream.buffer_ready_sem);
	if (result != B_OK) {
		dprintf("wii_audio: %s, Could not get playback buffer\n", __func__);
		return result;
	}

	result = acquire_sem(device->record_stream.buffer_ready_sem);
	if (result != B_OK) {
		dprintf("wii_audio: %s, Could not get record buffer\n", __func__);
		return result;
	}
	...
	if (((debug_buffers_exchanged % 5000) == 0) ) {
		dprintf("wii_audio: %s: %d buffers processed\n",
