...
static status_t
wii_audio_open (const char *name, uint32 flags, void** cookie)
{
	dprintf("wii_audio: %s\n" , __func__ );
	*cookie = &device;
	return B_OK;
...
static status_t
wii_audio_read (void* cookie, off_t a, void* b, size_t* num_bytes)
{
	dprintf("wii_audio: %s\n" , __func__ );
...
static status_t
wii_audio_write (void* cookie, off_t a, const void* b, size_t* num_bytes)
{
	dprintf("wii_audio: %s\n" , __func__ );
	// Audio drivers are not supposed to return anything
	// inside here
...
static status_t
wii_audio_control (void* cookie, uint32 op, void* arg, size_t len)
{
	//dprintf("wii_audio: %s\n" , __func__ );
	...
	if (cookie)
		...
		dprintf("wii_audio: %s called without cookie\n" , __func__);
